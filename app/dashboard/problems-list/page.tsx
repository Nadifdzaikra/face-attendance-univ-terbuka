...
export default function ProblemsListPage() {
  const [allData, setAllData] = useState<Problem[]>([]);
    ...
    try {
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          if (Array.isArray(maybeArray)) setAllData(maybeArray as Problem[]);
          else setAllData([]);
        }
      } else {
        setAllData([]);
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-zinc-900 dark:text-white">Laporan Masalah</h1>
            <p className="mt-2 text-zinc-600 dark:text-zinc-400">
              Kelola dan lihat semua laporan masalah yang telah diajukan
            </p>
          </div>
          <Link
            href="/problems"
            className="flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-all hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600"
          >
        ...
        <div className="border-b border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-900/50 sm:px-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h3 className="text-sm font-bold text-zinc-900 dark:text-white sm:text-base">
              Daftar Laporan ({pagination.total})
            </h3>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-700">
              {loading ? (
                  ...
                  <td colSpan={4} className="px-4 py-8 text-center sm:px-6">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-zinc-500 dark:text-zinc-500">
                        {searchTerm ? "Coba ubah kata kunci pencarian" : "Mulai buat laporan baru"}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                data.map((record, index) => (
                  <tr key={record.id} className="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-900/30">
