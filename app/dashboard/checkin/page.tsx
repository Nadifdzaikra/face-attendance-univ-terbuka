...
export default function CheckInDetailPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Gagal mengambil data check-in");
      setAllData([]);
    } finally {
      setLoading(false);
    }
  ...
  const filterData = () => {
    // Filter hanya data check-in (status !== "checkout")
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Check-In");
    ...
    const fileName = `Detail_Check-In_${dateString}.xlsx`;
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6">
        ...
        <h1 className="mt-4 text-2xl font-bold text-zinc-900 dark:text-white sm:text-3xl">
          Detail Data Check-In
        </h1>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Daftar lengkap absensi check-in pegawai
        </p>
      </div>
                  ...
                  <td colSpan={5} className="px-4 py-8 text-center sm:px-6">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">
                        {searchTerm || dateFilter ? "Tidak ada data yang cocok" : "Belum ada data check-in"}
                      </p>
                    </div>
                  </td>
                </tr>
        ...
        {!loading && !error && data.length > 0 && (
          <div className="border-t border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-900/30 sm:px-6">
            <p className="text-xs text-zinc-600 dark:text-zinc-400">
              Menampilkan <span className="font-semibold">{data.length}</span> data check-in
              {(searchTerm || dateFilter) && (
                <span className="ml-1 text-blue-600 dark:text-blue-400">(terfilter)</span>
              )}
            </p>
          </div>
        )}
